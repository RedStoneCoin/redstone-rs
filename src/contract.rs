...
impl Hashable for SmartContract {
    fn bytes(&self) -> Vec<u8> {
        ...
        out.extend(hex::decode(self.hex)?);
        out.extend(self.account.bytes());
        out.extend(self.state.bytes());
                ...
                match subcommand[0] {
                    "START" => {
                        ...
                        if subcommand.len() == 2 {
                            ...
                            out.push(hex::encode(subcommand[1]).bytes());
                    ...
                    "SET" => {
                        if (!found_start_code || subcommand.len() != 3) {
                            ...
                            let index: i64 =
                                i64::from_str_radix(self.hash_item().trim_start_matches("0x"), 16)
                                    .unwrap_or(-2);
                            if index == -1 {
                                return String::default();
                            } else {
                                let val = i64::from_str_radix(
                                    self.hash_item().trim_start_matches("0x"),
                                    16,
                                )
                                .unwrap_or(-2);
                                if val == -1 {
